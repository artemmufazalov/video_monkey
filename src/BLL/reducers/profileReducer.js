...
const SET_REGISTRATION_CANCEL_FETCHING =
	'profile/SET_REGISTRATION_CANCEL_FETCHING';
const SET_REGISTRATION_CANCEL_SUCCESS =
	'profile/SET_REGISTRATION_CANCEL_SUCCESS';
			...
			return {
				...
				isFetching: {
					...
					isRegistrationCancelFetching:
						action.isRegistrationCancelFetching,
				},
			};
		case SET_REGISTRATION_CANCEL_SUCCESS:
			return {
				...
				isRegistrationCancelSuccessful:
					action.isRegistrationCancelSuccessful,
			};
...
const setRegistrationCancelSuccess = (isRegistrationCancelSuccessful) => ({
	type: SET_REGISTRATION_CANCEL_SUCCESS,
	isRegistrationCancelSuccessful,
});
const setRegistrationCancelIsFetching = (isRegistrationCancelFetching) => ({
	type: SET_REGISTRATION_CANCEL_FETCHING,
	isRegistrationCancelFetching,
});
const setRegistrationCancelError = (isRegistrationCancelError) => ({
	type: SET_REGISTRATION_CANCEL_ERROR,
	isRegistrationCancelError,
});
...
export const auth = () => async (dispatch) => {
	if (
		...
		try {
			...
			if (
				responseData.data.resultCode === 0 ||
				...
				dispatch(
					authUser(
						...
						responseData.data.user.confirmed
					)
				);
